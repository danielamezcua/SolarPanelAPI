...
def read_voltage():
    ...
    voltage= float(subprocess.check_output(["python", "voltage.py"]).decode('utf-8').rstrip())
...
def move_solar_panel_y(userid,y):
    ...
    GPIO.setup(13,GPIO.OUT)    #Ponemos el pin 11 como salida
    p = GPIO.PWM(13,50)
    p.start(7.5)  #Ponemos el pin 11 en modo PWM y enviamos 50 pulsos por segundo
    p.ChangeDutyCycle(y)
...
def search_max_power(userid):
    ...
    x = GPIO.PWM(11,50)        #Ponemos el pin 21 en modo PWM y enviamos 50 pulsos por segundo
